...
namespace CUSTIS.I18N.DAL
{
    ...
    public sealed class XmlFormatter : IFormatter
    {
        ...
        private void Serialize(XmlWriter writer, IFormatterConverter converter, string elementName, object objectToSerialize, Type objectType)
        {
            // Include type information when using ISerializable.
            bool includeTypeInfo = (objectToSerialize is ISerializable);

            // write the opening tag
            writer.WriteStartElement(elementName, _ns);
        ...
        private void WriteAttributes(XmlWriter writer, Type objectType, bool includeArrayAttribute)
        {
        ...
        private void WriteValueElement(XmlWriter writer, CustomSerializationEntry entry)
        {
        ...
        /// <param name="writer">The XmlWriter to write to.</param>
        /// <param name="entry">The entry to write to the element.</param>
        private void WriteValueElement(string tagName, XmlWriter writer, CustomSerializationEntry entry)
        {
            // write opening tag
        ...
        private IEnumerable<CustomSerializationEntry> GetMemberInfo(object objectToSerialize, Type objectToSerializeType, IFormatterConverter converter)
        {
        ...
        private object InitializeObject(XmlReader reader, IFormatterConverter converter, Type objectType)
        {
            Type actualType;
            ISurrogateSelector selector1 = null;
            ISerializationSurrogate serializationSurrogate = null;
            ...
            else if (typeof(ISerializable).IsAssignableFrom(actualType))
            {
                ...
                ConstructorInfo ctor = actualType.GetConstructor(BindingFlags.NonPublic | BindingFlags.Instance, null, new Type[] { typeof(SerializationInfo), typeof(StreamingContext) }, null);
                if (ctor == null)
                    throw new SerializationException(string.Format("{0} deserialization ctor not found", actualType));
                // Create the object
        ...
        private void PopulateSerializationInfo(XmlReader reader, IFormatterConverter converter, Type actualType, SerializationInfo info)
        {
            // read the next element.
            reader.ReadStartElement();
        ...
        private object DetermineValue(XmlReader reader, IFormatterConverter converter, Type objectType)
        {
            object parsedObject;
